...
export default async function Achievements({ params, searchParams }) {
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      <ul role="list" className="w-full max-w-screen-sm p-8 bg-white rounded-xl shadow-md divide-y divide-gray-200">
        {achievements.map((achievement) => {
          let unlocked = savedata.achievements[`${achievement.id}`].unlocked;
          if (filtered && unlocked) return;
          return (
            <li key={achievement.id} className="flex justify-between gap-x-6 py-5">
              <div className="flex min-w-0 gap-x-4">
                <div className="min-w-0 flex-auto">
                  <p className="text-sm font-semibold leading-6 text-gray-900">{achievement.name}</p>
